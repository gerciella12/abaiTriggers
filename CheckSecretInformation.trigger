...
trigger CheckSecretInformation on Case (after insert, before update) {
    ...
    //1. Create a collection containing each of or secret keywords
    ...
    //2. Check to see if our case contains any of the secret keywords
    ...
    //3. If contains, create a child case
    List <Case> casesToCreate = new List<Case>();
        for(Case caseWithSecretInformation : casesWithSecretInfo) {
            Case childCase         = new Case();
            childCase.Subject      = 'Warning: Parent may contain secret info';
            childCase.ParentId     = caseWithSecretInformation.Id;
